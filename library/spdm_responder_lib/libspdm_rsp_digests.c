                                              ...
                                              void *response)
{
    ...
    for (index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        if (spdm_context->local_context
            .local_cert_chain_provision[spdm_context->local_context.current_bank][index] != NULL) {
        ...
        if (spdm_context->local_context
            ...
            if ((spdm_request->header.spdm_version >= SPDM_MESSAGE_VERSION_13) &&
                ...
                key_pair_id[slot_index] =
                    spdm_context->local_context.local_key_pair_id[spdm_context->local_context.current_bank][index];
                cert_info[slot_index] =
                    spdm_context->local_context.local_cert_info[spdm_context->local_context.current_bank][index];
                key_usage_bit_mask[slot_index] =
                    spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.current_bank][index];
            }
            slot_index++;
            if (!result) {
                return libspdm_generate_error_response(
