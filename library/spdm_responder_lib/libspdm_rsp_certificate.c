                                                  ...
                                                  void *response)
{
    ...
    if (spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][slot_id] ==
        NULL) {
        return libspdm_generate_error_response(
    ...
    *response_size = rsp_msg_header_size + length;
    ...
    if ((spdm_request->header.spdm_version >= SPDM_MESSAGE_VERSION_13) &&
        ...
        spdm_response->header.param2 =
            spdm_context->local_context.local_cert_info[spdm_context->local_context.current_bank][slot_id];
    }
    if (use_large_cert_chain) {
    ...
    libspdm_copy_mem((uint8_t *)spdm_response + rsp_msg_header_size,
                     ...
                     .local_cert_chain_provision[spdm_context->local_context.current_bank][slot_id] + offset, length);
