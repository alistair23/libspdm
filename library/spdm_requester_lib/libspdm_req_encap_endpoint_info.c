                                                          ...
                                                          void *response)
{
    ...
    if ((spdm_request->request_attributes &
        ...
        if (slot_id != 0xF) {
            if (context->local_context.local_cert_chain_provision[context->local_context.current_bank][slot_id] ==
                NULL) {
                return libspdm_generate_encap_error_response(
                    context, SPDM_ERROR_CODE_INVALID_REQUEST,
                    0, response_size, response);
        ...
        if (context->connection_info.multi_key_conn_req && slot_id != 0xF) {
            if ((context->local_context.local_key_usage_bit_mask[context->local_context.current_bank][slot_id] &
