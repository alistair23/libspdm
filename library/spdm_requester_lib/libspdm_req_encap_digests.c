    ...
    *response_size = sizeof(spdm_digest_response_t) + (hash_size + additional_size) * slot_count;
    ...
    for (index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        if (context->local_context
            .local_cert_chain_provision[context->local_context.current_bank][index] != NULL) {
            ...
            if ((spdm_request->header.spdm_version >= SPDM_MESSAGE_VERSION_13) &&
                ...
                key_pair_id[slot_index] =
                    context->local_context.local_key_pair_id[context->local_context.current_bank][index];
                cert_info[slot_index] =
                    context->local_context.local_cert_info[context->local_context.current_bank][index];
                key_usage_bit_mask[slot_index] =
                    context->local_context.local_key_usage_bit_mask[context->local_context.current_bank][index];
            }
            slot_index++;
            if (!result) {
                return libspdm_generate_encap_error_response(
