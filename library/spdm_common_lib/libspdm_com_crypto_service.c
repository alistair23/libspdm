    ...
    bool is_requester)
{
    ...
    return context->local_context.local_key_pair_id[context->local_context.current_bank][slot_id];
}
                                         ...
                                         size_t *cert_chain_buffer_size)
{
    ...
    LIBSPDM_ASSERT(context->local_context.local_cert_chain_provision[context->local_context.current_bank][slot_id] !=
                   NULL);
    LIBSPDM_ASSERT(context->local_context.local_cert_chain_provision_size[context->local_context.current_bank] != 0);

    *cert_chain_buffer =
        context->local_context.local_cert_chain_provision[context->local_context.current_bank][slot_id];
                                      ...
                                      size_t slot_id, uint8_t *hash)
{
    ...
    return libspdm_hash_all(
        spdm_context->connection_info.algorithm.base_hash_algo,
        spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][slot_id],
        spdm_context->local_context.local_cert_chain_provision_size[spdm_context->local_context.current_bank][slot_id],
        hash);
}
