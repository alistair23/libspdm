                                  ...
                                  size_t data_size)
{
    ...
    case LIBSPDM_DATA_LOCAL_PUBLIC_CERT_CHAIN:
        ...
        context->local_context.local_cert_chain_provision_size[context->local_context.current_bank][slot_id] =
            data_size;
        context->local_context.local_cert_chain_provision[context->local_context.current_bank][slot_id] = data;
        break;
    case LIBSPDM_DATA_LOCAL_CURRENT_BANK:
        ...
        context->local_context.current_bank = data8;
        break;
    ...
    case LIBSPDM_DATA_LOCAL_KEY_PAIR_ID:
        ...
        context->local_context.local_key_pair_id[context->local_context.current_bank][slot_id] =
    ...
    case LIBSPDM_DATA_LOCAL_CERT_INFO:
        ...
        context->local_context.local_cert_info[context->local_context.current_bank][slot_id] =
    ...
    case LIBSPDM_DATA_LOCAL_KEY_USAGE_BIT_MASK:
        ...
        context->local_context.local_key_usage_bit_mask[context->local_context.current_bank][slot_id] =
...
bool libspdm_check_context (void *spdm_context)
{
    ...
    if (((context->local_context.capability.flags &
        ...
        for (index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
            if ((context->local_context.local_cert_chain_provision_size[context->local_context.current_bank][index] !=
                 0) &&
                (context->local_context.local_cert_chain_provision_size[context->local_context.current_bank][index] +
                 sizeof(spdm_certificate_response_t) >
                 context->local_context.capability.max_spdm_msg_size)) {
                LIBSPDM_DEBUG((LIBSPDM_DEBUG_ERROR,
                               "max_spdm_msg_size (%d) must be greater than or "
                               "equal to local_cert_chain_provision_size[context->local_context.current_bank][%zu] (%zu).\n",
                               context->local_context.capability.max_spdm_msg_size, index,
                               context->local_context.local_cert_chain_provision_size[context->local_context.
                                                                                      current_bank][index]));
                return false;
            }
        }
