...
static void rsp_challenge_auth_case1(void **state)
{
    ...
    spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][0] = data1;
...
static void rsp_challenge_auth_case9(void **state) {
    ...
    libspdm_read_responder_public_certificate_chain (m_libspdm_use_hash_algo,
                                                     ...
                                                     &data1, &data_size1,
                                                     NULL, NULL);
    spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][1] = data1;
    spdm_context->local_context.local_cert_chain_provision_size[spdm_context->local_context.current_bank][1] =
        data_size1;
    libspdm_secret_lib_challenge_opaque_data_size = 0;
    libspdm_reset_message_c(spdm_context);
    ...
    for (index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        if (spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][index] !=
            NULL) {
            slot_mask |= (1 << index);
        }
    }
...
static void rsp_challenge_auth_case10(void **state) {
    ...
    libspdm_read_responder_public_certificate_chain (m_libspdm_use_hash_algo,
                                                     ...
                                                     &data1, &data_size1,
                                                     NULL, NULL);
    /*clear local_cert_chain_provision[context->local_context.current_bank]*/
    for (index = 0; index <SPDM_MAX_SLOT_COUNT; index++) {
        spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][index] = NULL;
        spdm_context->local_context.local_cert_chain_provision_size[spdm_context->local_context.current_bank][index] =
            0;
    }
...
static void rsp_challenge_auth_case19(void **state)
{
    ...
    spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.current_bank][slot_id] =
