...
static void rsp_endpoint_info_case1(void **state)
{
    ...
    for (int i = 0; i < SPDM_MAX_SLOT_COUNT; i++) {
        spdm_context->local_context.local_key_pair_id[spdm_context->local_context.current_bank][i] = 1;
...
static void rsp_endpoint_info_case3(void **state)
{
    ...
    spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.current_bank][1] =
