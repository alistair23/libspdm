...
void libspdm_test_responder_endpoint_info_err_case13(void **state)
{
    ...
    libspdm_read_responder_public_certificate_chain(m_libspdm_use_hash_algo,
                                                    ...
                                                    &data_size, NULL, NULL);
    for (int i = 0; i < SPDM_MAX_SLOT_COUNT; i++) {
        spdm_context->local_context.local_cert_chain_provision_size[spdm_context->local_context.current_bank][i] =
            data_size;
        spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][i] = data;
    }
    ...
    spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.current_bank][1] = 0;
