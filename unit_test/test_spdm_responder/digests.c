...
static void rsp_digests_case1(void **state)
{
    ...
    spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][0] =
...
static void rsp_digests_case7(void **state)
{
    ...
    for (index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        ...
        .local_cert_chain_provision_size[spdm_context->local_context.current_bank][index] = 0;
    }
...
static void rsp_digests_case10(void **state)
{
    ...
    for (uint8_t index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][index] =
    ...
    for (uint8_t index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        assert_memory_equal((void *)&key_pair_id[index],
                            (void *)&spdm_context->local_context.local_key_pair_id[spdm_context->local_context.
                                                                                   current_bank][
                                index],
                            sizeof(spdm_key_pair_id_t));
        assert_memory_equal((void *)&cert_info[index],
                            (void *)&spdm_context->local_context.local_cert_info[spdm_context->local_context.
                                                                                 current_bank][index],
                            sizeof(spdm_certificate_info_t));
        assert_memory_equal((void *)&key_usage_bit_mask[index],
                            (void *)&spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.
                                                                                          current_bank][index],
                            sizeof(spdm_key_usage_bit_mask_t));
    }
}
