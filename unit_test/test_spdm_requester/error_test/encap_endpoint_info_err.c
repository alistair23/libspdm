 ...
 *         but local_cert_chain_provision[context->local_context.current_bank][slot_id] == NULL
 * Expected Behavior: generate an ERROR_RESPONSE with code SPDM_ERROR_CODE_INVALID_REQUEST
 **/
void libspdm_test_requester_encap_endpoint_info_err_case7(void **state)
{
    ...
    /* no initialization for spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank] */
    for (int i = 0; i < SPDM_MAX_SLOT_COUNT; i++) {
        spdm_context->local_context.local_cert_chain_provision_size[spdm_context->local_context.current_bank][i] = 0;
        spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][i] = NULL;
    }
 ...
 *         but local_key_usage_bit_mask[context->local_context.current_bank][slot_id] not meet requirement
 * Expected Behavior: generate an ERROR_RESPONSE with code SPDM_ERROR_CODE_INVALID_REQUEST
 **/
void libspdm_test_requester_encap_endpoint_info_err_case9(void **state)
{
    ...
    for (int i = 0; i < SPDM_MAX_SLOT_COUNT; i++) {
        spdm_context->local_context.local_cert_chain_provision_size[spdm_context->local_context.current_bank][0] =
            data_size;
        spdm_context->local_context.local_cert_chain_provision[spdm_context->local_context.current_bank][0] = data;
#if LIBSPDM_RECORD_TRANSCRIPT_DATA_SUPPORT
    ...
    /* no initialization for spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.current_bank] */
    for (int i = 0; i < SPDM_MAX_SLOT_COUNT; i++) {
        spdm_context->local_context.local_key_usage_bit_mask[spdm_context->local_context.current_bank][i] = 0;
    }
...
int libspdm_req_encap_endpoint_info_error_test(void)
{
    const struct CMUnitTest test_cases[] = {
        ...
        /* Signature was required but local_cert_chain_provision[context->local_context.current_bank][slot_id] == NULL */
         ...
         * local_key_usage_bit_mask[context->local_context.current_bank][slot_id] not meet requirement
         */
