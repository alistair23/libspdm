    ...
    void *spdm_context, size_t *response_size, void **response, uint64_t timeout)
{
    ...
    case 0x1B: {
        ...
        for (uint8_t index = 0; index < slot_count; index++)
        {
            ...
            key_pair_id[index] = m_local_key_pair_id[context->local_context.current_bank][index];
            cert_info[index] = m_local_cert_info[context->local_context.current_bank][index];
            key_usage_bit_mask[index] = m_local_key_usage_bit_mask[context->local_context.current_bank][index];
    ...
    case 0x1C: {
        ...
        libspdm_zero_mem (digest, hash_size * slot_count);
        key_pair_id[0] = m_local_key_pair_id[context->local_context.current_bank][0];
        cert_info[0] = m_local_cert_info[context->local_context.current_bank][0];
        key_usage_bit_mask[0] = m_local_key_usage_bit_mask[context->local_context.current_bank][0];
    ...
    case 0x1D: {
        ...
        key_pair_id[1] = m_local_key_pair_id[context->local_context.current_bank][1];
        cert_info[1] = m_local_cert_info[context->local_context.current_bank][1];
        key_usage_bit_mask[1] = m_local_key_usage_bit_mask[context->local_context.current_bank][1];
...
static void req_get_digests_case27(void **state)
{
    ...
    m_local_key_pair_id[spdm_context->local_context.current_bank][0] = 0x00;
    m_local_cert_info[spdm_context->local_context.current_bank][0] = SPDM_CERTIFICATE_INFO_CERT_MODEL_DEVICE_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][0] = SPDM_KEY_USAGE_BIT_MASK_KEY_EX_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][1] = 0x01;
    m_local_cert_info[spdm_context->local_context.current_bank][1] = SPDM_CERTIFICATE_INFO_CERT_MODEL_ALIAS_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][1] = SPDM_KEY_USAGE_BIT_MASK_CHALLENGE_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][2] = 0x02;
    m_local_cert_info[spdm_context->local_context.current_bank][2] = SPDM_CERTIFICATE_INFO_CERT_MODEL_GENERIC_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][2] = SPDM_KEY_USAGE_BIT_MASK_MEASUREMENT_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][3] = 0x03;
    m_local_cert_info[spdm_context->local_context.current_bank][3] = SPDM_CERTIFICATE_INFO_CERT_MODEL_DEVICE_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][3] = SPDM_KEY_USAGE_BIT_MASK_ENDPOINT_INFO_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][4] = 0x04;
    m_local_cert_info[spdm_context->local_context.current_bank][4] = SPDM_CERTIFICATE_INFO_CERT_MODEL_DEVICE_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][4] = SPDM_KEY_USAGE_BIT_MASK_STANDARDS_KEY_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][5] = 0x05;
    m_local_cert_info[spdm_context->local_context.current_bank][5] = SPDM_CERTIFICATE_INFO_CERT_MODEL_GENERIC_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][5] = SPDM_KEY_USAGE_BIT_MASK_VENDOR_KEY_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][6] = 0x06;
    m_local_cert_info[spdm_context->local_context.current_bank][6] = SPDM_CERTIFICATE_INFO_CERT_MODEL_GENERIC_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][6] = SPDM_KEY_USAGE_BIT_MASK_VENDOR_KEY_USE;

    m_local_key_pair_id[spdm_context->local_context.current_bank][7] = 0x07;
    m_local_cert_info[spdm_context->local_context.current_bank][7] = SPDM_CERTIFICATE_INFO_CERT_MODEL_GENERIC_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][7] = SPDM_KEY_USAGE_BIT_MASK_VENDOR_KEY_USE;
    ...
    for (uint8_t index = 0; index < SPDM_MAX_SLOT_COUNT; index++) {
        assert_memory_equal((void *)&m_local_key_pair_id[spdm_context->local_context.current_bank][index],
                            (void *)&spdm_context->connection_info.peer_key_pair_id[index],
                            sizeof(spdm_key_pair_id_t));
        assert_memory_equal((void *)&m_local_cert_info[spdm_context->local_context.current_bank][index],
                            (void *)&spdm_context->connection_info.peer_cert_info[index],
                            sizeof(spdm_key_pair_id_t));
        assert_memory_equal((void *)&m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][index],
...
static void req_get_digests_case28(void **state)
{
    ...
    m_local_key_pair_id[spdm_context->local_context.current_bank][0] = 0x00;
    m_local_cert_info[spdm_context->local_context.current_bank][0] = SPDM_CERTIFICATE_INFO_CERT_MODEL_DEVICE_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][0] = 0;
    ...
    m_local_key_pair_id[spdm_context->local_context.current_bank][0] = 0x00;
    m_local_cert_info[spdm_context->local_context.current_bank][0] = SPDM_CERTIFICATE_INFO_CERT_MODEL_GENERIC_CERT;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][0] = SPDM_KEY_USAGE_BIT_MASK_ENDPOINT_INFO_USE;
...
static void req_get_digests_case29(void **state)
{
     ...
     * MULTI_KEY_CONN_REQ or MULTI_KEY_CONN_RSP is false. */
    m_local_key_pair_id[spdm_context->local_context.current_bank][1] = 0x01;
    m_local_cert_info[spdm_context->local_context.current_bank][1] = SPDM_CERTIFICATE_INFO_CERT_MODEL_NONE;
    m_local_key_usage_bit_mask[spdm_context->local_context.current_bank][1] = SPDM_KEY_USAGE_BIT_MASK_ENDPOINT_INFO_USE;
