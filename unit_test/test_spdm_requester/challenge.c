    ...
    void *spdm_context, size_t *response_size, void **response, uint64_t timeout)
{
    ...
    case 0x2: { /*correct CHALLENGE_AUTH message*/
        ...
        ->local_context.local_cert_chain_provision_size[context->local_context.current_bank][0] = data_size;
        ((libspdm_context_t *)spdm_context)
        ->local_context.local_cert_chain_provision[context->local_context.current_bank][0] = data;
        ((libspdm_context_t *)spdm_context)
        ...
        libspdm_hash_all(
            ...
            ((libspdm_context_t *)spdm_context)
            ->local_context.local_cert_chain_provision[context->local_context.current_bank][0],
            ((libspdm_context_t *)spdm_context)
            ->local_context
            .local_cert_chain_provision_size[context->local_context.current_bank][0],
            ptr);
        free(data);
    ...
    case 0x6: { /*correct ERROR message (busy) + correct CHALLENGE_AUTH message*/
        ...
        if (sub_index1 == 0) {
            ...
            ((libspdm_context_t *)spdm_context)
            ->local_context
            .local_cert_chain_provision_size[context->local_context.current_bank][0] = data_size;
            ((libspdm_context_t *)spdm_context)
            ->local_context.local_cert_chain_provision[context->local_context.current_bank][0] = data;
            ((libspdm_context_t *)spdm_context)
            ->connection_info.algorithm.base_asym_algo = m_libspdm_use_asym_algo;
            ...
            libspdm_hash_all(
                ...
                ((libspdm_context_t *)spdm_context)
                ->local_context
                .local_cert_chain_provision[context->local_context.current_bank][0],
                ((libspdm_context_t *)spdm_context)
                ->local_context
                .local_cert_chain_provision_size[context->local_context.current_bank][0],
                ptr);
    ...
    case 0xA: /*correct CHALLENGE_AUTH message*/
    {
        ...
        libspdm_read_responder_public_certificate_chain (m_libspdm_use_hash_algo,
                                                         ...
                                                         &data,
                                                         &data_size, NULL, NULL);
        ((libspdm_context_t*)spdm_context)->local_context.local_cert_chain_provision_size[context->local_context.
                                                                                          current_bank][0] =
            data_size;
        ((libspdm_context_t*)spdm_context)->local_context.local_cert_chain_provision[context->local_context.current_bank]
        [0] = data;
    ...
    case 0x16: { /*correct CHALLENGE_AUTH message*/
        ...
        ((libspdm_context_t *)spdm_context)->local_context.local_cert_chain_provision_size[context->local_context.
                                                                                           current_bank][0] =
            data_size;
        ((libspdm_context_t *)spdm_context)->local_context.local_cert_chain_provision[context->local_context.
                                                                                      current_bank][0] = data;
        ((libspdm_context_t *)spdm_context)->connection_info.algorithm.base_asym_algo =
            m_libspdm_use_asym_algo;
        ((libspdm_context_t *)spdm_context)->connection_info.algorithm.base_hash_algo =
        ...
        libspdm_hash_all(m_libspdm_use_hash_algo,
                         ...
                         ->local_context.local_cert_chain_provision[context->local_context.current_bank][0],
                         ((libspdm_context_t *)spdm_context)
                         ->local_context.local_cert_chain_provision_size[context->local_context.current_bank][0],
                         ptr);
        free(data);
        ptr += libspdm_get_hash_size(m_libspdm_use_hash_algo);
        libspdm_get_random_number(SPDM_NONCE_SIZE, ptr);
    ...
    case 0x17: { /*correct CHALLENGE_AUTH message*/
        ...
        ((libspdm_context_t *)spdm_context)
        ->local_context.local_cert_chain_provision_size[context->local_context.current_bank][slot_id] = data_size;
        ((libspdm_context_t *)spdm_context)
        ->local_context.local_cert_chain_provision[context->local_context.current_bank][slot_id] = data;
        ((libspdm_context_t *)spdm_context)
        ->connection_info.algorithm.base_asym_algo = m_libspdm_use_asym_algo;
        ((libspdm_context_t *)spdm_context)
        ->connection_info.algorithm.base_hash_algo = m_libspdm_use_hash_algo;
        ...
        libspdm_hash_all(
            m_libspdm_use_hash_algo,
            ((libspdm_context_t *)spdm_context)
            ->local_context.local_cert_chain_provision[context->local_context.current_bank][slot_id],
            ((libspdm_context_t *)spdm_context)
            ->local_context
            .local_cert_chain_provision_size[context->local_context.current_bank][slot_id],
            ptr);
        free(data);
        ptr += libspdm_get_hash_size(m_libspdm_use_hash_algo);
        libspdm_get_random_number(SPDM_NONCE_SIZE, ptr);
